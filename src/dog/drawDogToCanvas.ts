...
// This is originally just CanvasImageSource, but annoyingly VideoFrame doesn't have
// `width` or `height` parameters, it also rejects node-canvas :(
export type CanvasImageSourceWithSize = HTMLImageElement;

const HEAD_BIG_SIZE = 480;
function getHeadOrigin(head: CanvasImageSourceWithSize): [number, number] {
...
export default function drawDogToCanvas(
	...
	images: {
		idle2: CanvasImageSourceWithSize;
		clothes: CanvasImageSourceWithSize;
		idle1: CanvasImageSourceWithSize;
		clothesLayer2: CanvasImageSourceWithSize | null;
		head: CanvasImageSourceWithSize;
		headSkin: CanvasImageSourceWithSize | null;
		hair: CanvasImageSourceWithSize;
		ear: CanvasImageSourceWithSize;
	},
	colors: {
		clothesColor: string;
		earColor: string;
	...
	options: {
		...
		hats: ReadonlyArray<{
			...
			hatInfo: DogHatType;
			// Images
			hatShowHairExtra: CanvasImageSourceWithSize | null;
			hat: CanvasImageSourceWithSize | null;
			hatLayer2: CanvasImageSourceWithSize | null;
		}>;
		showBody: boolean;
	}
) {
	...
	const clothesCollar: number | null = gotAnyHatWithLayer2
		? null
		: (options.clothesInfo.collar ?? null);
	...
	drawImageAsColor(
		...
		images.head,
		{
			...
			customOverlayImage: images.headSkin,
			fillColor: colors.skinColor,
			outlineColor: colors.skinOutlineColor,
		},
		headTranslatedX,
